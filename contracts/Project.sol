...
contract Project {
    ...
    address payable public developer =
        payable(0x75BF6c63F2941ED31A908B976b5E39954561517b);
    ...
    modifier isMasterContract() {
        require(
            ...
            "You do not have the master rights to perform this operation!"
        );
        _;
    }
    ...
    modifier isAdmin() {
        require(
            msg.sender == admin,
            "You dont have admin access to perform this operation !"
        );
        _;
    }
    ...
    modifier isCreator() {
        require(
            msg.sender == creator,
            "You dont have creator access to perform this operation !"
        );
        _;
    }
    ...
    modifier isOriginalSenderCreator(address senderAddress) {
        require(
            ...
            "Original sender is not creator and cannot perform this operation !"
        );
        _;
    }
    ...
    function contribute(
        ...
        uint256 amount,
        IERC20 tokenAddress
    ) public isProjectVerified isMasterContract {
    ...
    function withdrawRequestedAmount(
        ...
        IERC20 _usdt,
        IERC20 _usdc
    ) external isCreator validateExpiry(State.Successful) {
        WithdrawRequest storage requestDetails = withdrawRequests[_requestId];
        require(
        ...
        require(
            requestDetails.noOfVotes >= noOfContributers / 5,
            "At least 20% contributor need to vote for this request"
        );
        require(
            requestDetails.amountUSDT <= _usdt.balanceOf(address(this)),
            "Project contract does not have enough USDT balance to disburse"
        );
        require(
            requestDetails.amountUSDC <= _usdc.balanceOf(address(this)),
            "Project contract does not have enough USDC balance to disburse"
        );

        _usdt.transferFrom(
            address(this),
            requestDetails.reciptent,
            (requestDetails.amountUSDT / 50) * 49
        );
        _usdc.transferFrom(
            address(this),
            requestDetails.reciptent,
            (requestDetails.amountUSDC / 50) * 49
        );
        ...
        if (block.timestamp <= royaltyGraceExpiry) {
            ...
            _usdt.transferFrom(
                address(this),
                admin,
                requestDetails.amountUSDT / 50
            );
            _usdc.transferFrom(
                address(this),
                admin,
                requestDetails.amountUSDC / 50
            );
        }
